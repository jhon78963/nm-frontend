...
/** Árbol alineado con el menú principal y sus submódulos. */
  ...
  return tree
    .map(moduleEntry => {
      ...
      const submodules = moduleEntry.submodules
        ...
        .filter(
          (submoduleEntry): submoduleEntry is PermissionSubmodule =>
            submoduleEntry !== null,
        );
    ...
    .filter(
      (moduleEntry): moduleEntry is PermissionModule => moduleEntry !== null,
    );
}
  ...
  return tree.reduce(
    (total, moduleEntry) =>
      ...
      moduleEntry.submodules.reduce(
        (subTotal, submoduleEntry) =>
          subTotal + submoduleEntry.permissions.length,
        0,
      ),
    0,
  );
...
function resolvePermissionPlacement(perm: Permission): MenuPlacement {
  ...
  const byPrefix: Record<string, MenuPlacement> = {
    ...
    productImage: placement('Inventario', 'Productos · Imágenes', 3),
    productHistory: placement('Inventario', 'Productos · Historial', 4),
    inventoryKardex: placement('Inventario', 'Productos · Kardex', 5),
    size: placement('Inventario', 'Tallas', 6),
    color: placement('Inventario', 'Colores', 7),
    inventoryReconciliation: placement('Inventario', 'Cuadre rápido', 8),
    image: placement('Inventario', 'Imágenes (catálogo)', 9),
    gender: placement('Inventario', 'Géneros', 10),
  ...
  const fallbackSubmodule = perm.group?.trim() || prefix;

  return {
    module: 'Otros',
    submodule: fallbackSubmodule,
